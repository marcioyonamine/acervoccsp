...
function verificaMysql($sql_inserir){ 	//Verifica erro na string/query
	$mysqli = new mysqli("200.237.5.34", "root", "lic54eca","acervo");
...
function geraOpcaoRelJuridica($select,$idUsuario){ //gera os options de um select
	$con = bancoMysqli();
...
function listaArquivosRegistro($id_registro){ //lista arquivos de determinado evento
	$con = bancoMysqli();
...
function retornaData($id){ //retorna o período
	$con = bancoMysqli();
...
function reloadAgenda(){
	...
	while($evento = mysqli_fetch_array($query_pesquisar)){
		...
		if($evento['virada'] == 1){
			$sql_virada = "INSERT INTO `igsis_agenda` (`idAgenda`, `idEvento`, `data`, `hora`, `idLocal`, `idInstituicao`, `idTipo`, `idOcorrencia`, `idCinema`) 
		VALUES (NULL, '$idEvento', '2016-05-22', '00:00:00', '388', '4', '$idTipo', '$idOcorrencia', '$idCinema');";
			$query_virada = mysqli_query($con,$sql_virada);
			if($query){
...
$sql_pesquisar = "SELECT ig_ocorrencia.idEvento, dataInicio, idTipoOcorrencia, local, horaInicio, idInstituicao, dataFinal, segunda, terca, quarta, quinta, sexta, sabado, domingo, idOcorrencia, idCinema, virada, ocupacao, dataEnvio FROM ig_ocorrencia, ig_evento WHERE ig_evento.publicado = '1' AND ig_evento.publicado = '1' AND ig_evento.idEvento = ig_ocorrencia.idEvento AND ig_ocorrencia.publicado = '1' AND ig_evento.idEvento = '$idEvento' ORDER BY dataInicio, horaInicio";
$query_pesquisar = mysqli_query($con,$sql_pesquisar);
$mensagem = "Erro";
$data = "";
$data_antigo = "1";
...
function avisoComunicacao($idEvento){
	$con = bancoMysqli();
	$sql = "INSERT INTO ig_com_re_evento (`ig_produtor_idProdutor`, `ig_tipo_evento_idTipoEvento`, `ig_programa_idPrograma`, `projetoEspecial`, `nomeEvento`, `projeto`, `memorando`, `idResponsavel`, `suplente`, `autor`, `fichaTecnica`, `faixaEtaria`, `sinopse`, `releaseCom`, `parecerArtistico`, `confirmaFinanca`, `confirmaDiretoria`, `confirmaComunicacao`, `confirmaDocumentacao`, `confirmaProducao`, `numeroProcesso`, `publicado`, `idUsuario`, `ig_modalidade_IdModalidade`, `linksCom`, `subEvento`, `dataEnvio`, `justificativa`, `idInstituicao`, `ocupacao`, `idEvento`) SELECT `ig_produtor_idProdutor`, `ig_tipo_evento_idTipoEvento`, `ig_programa_idPrograma`, `projetoEspecial`, `nomeEvento`, `projeto`, `memorando`, `idResponsavel`, `suplente`, `autor`, `fichaTecnica`, `faixaEtaria`, `sinopse`, `releaseCom`, `parecerArtistico`, `confirmaFinanca`, `confirmaDiretoria`, `confirmaComunicacao`, `confirmaDocumentacao`, `confirmaProducao`, `numeroProcesso`, `publicado`, `idUsuario`, `ig_modalidade_IdModalidade`, `linksCom`, `subEvento`, `dataEnvio`, `justificativa`, `idInstituicao`, `ocupacao`, `idEvento` FROM ig_evento WHERE idEvento = '$idEvento'";
	$query = mysqli_query($con,$sql);	
	if($query){
		$sql_agenda = "INSERT INTO ig_com_re_agenda (`idAgenda`, `idEvento`, `data`, `hora`, `idLocal`, `idInstituicao`, `idTipo`, `idOcorrencia`, `idCinema`) SELECT `idAgenda`, `idEvento`, `data`, `hora`, `idLocal`, `idInstituicao`, `idTipo`, `idOcorrencia`, `idCinema` FROM igsis_agenda WHERE idEvento = '$idEvento'";
